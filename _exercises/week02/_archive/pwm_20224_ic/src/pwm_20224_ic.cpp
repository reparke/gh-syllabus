...
/*
  how could we adjust the brightness automatically (so the LED gets brighter on
  its own)? -counter that increments with time -loop
...
void loop() {
    // int brightness = 0;    this would be a LOCAL var that would get erased
    // each loop()
    // step 3: write to pin
    ...
    // delay(1000);
    // analogWrite(PIN_LED, 0);  // 0 brightness which the same as digitalWrite
    // (LOW) delay(1000);
