/*
 * RFID.cpp - Library to use ARDUINO RFID MODULE KIT 13.56 MHZ WITH TAGS SPI W
 * AND R BY COOQROBOT. Based on code Dr.Leong   ( WWW.B2CQSHOP.COM ) Created by
 * Miguel Balboa, Jan, 2012. Modified by Paul Kourany to run on Spark Core with
 * added support for Software SPI, Mar, 2014. Released into the public domain.
 */
...
RFID::RFID(uint8_t chipSelectPin, uint8_t NRSTPD) {
    ...
    pinMode(
        _chipSelectPin,
        OUTPUT);  // Set digital as OUTPUT to connect it to the RFID /ENABLE pin
...
RFID::RFID(uint8_t chipSelectPin, uint8_t NRSTPD, uint8_t mosiPin,
           uint8_t misoPin, uint8_t clockPin) {
    _mosiPin = mosiPin;
    _misoPin = misoPin;
    _clockPin = clockPin;
    _chipSelectPin = chipSelectPin;
...
uint8_t RFID::MFRC522ToCard(uint8_t command, uint8_t *sendData, uint8_t sendLen,
    ...
    clearBitMask(
        CommIrqReg,
        0x80);  // Borrar todos los bits de peticiÃƒÂ³n de interrupciÃƒÂ³n
    ...
    if (command == PCD_TRANSCEIVE) {
        setBitMask(BitFramingReg,
                   0x80);  // StartSend=1,transmission of data starts
    }

    // A la espera de recibir datos para completar
    i = 2000;  // i????????,??M1???????25ms	??? i De acuerdo con el ajuste
               // de frecuencia de reloj, el tiempo mÃƒÂ¡ximo de espera
               // operaciÃƒÂ³n M1 25ms tarjeta??
    do {
        // CommIrqReg[7..0]
 ...
 *  Anti-detecciÃƒÂ³n de colisiones, la lectura del nÃƒÂºmero de serie de la
 * tarjeta de tarjeta
 *  @param serNum - devuelve el nÃƒÂºmero de tarjeta 4 bytes de serie, los
 * primeros 5 bytes de bytes de paridad
 ...
 * Los parÃƒÂ¡metros de entrada: AuthMode - Modo de autenticaciÃƒÂ³n de
 contraseÃƒÂ±a 0x60 = A 0x60 = validaciÃƒÂ³n KeyA 0x61 = B 0x61 = validaciÃƒÂ³n
 KeyB BlockAddr--  bloque de direcciones Sectorkey-- sector contraseÃƒÂ±a
             serNum--,4? Tarjeta de nÃƒÂºmero de serie, 4 bytes
 * MI_OK Valor de retorno: el retorno exitoso MI_OK
 */
uint8_t RFID::auth(uint8_t authMode, uint8_t BlockAddr, uint8_t *Sectorkey,
                   uint8_t *serNum) {
    uint8_t status;
    uint16_t recvBits;
    uint8_t i;
    ...
    //????+???+????+???? Verifique la direcciÃƒÂ³n de comandos de bloques del
    //sector + + contraseÃƒÂ±a + nÃƒÂºmero de la tarjeta de serie
    buff[0] = authMode;
    buff[1] = BlockAddr;
    for (i = 0; i < 6; i++) {
 ...
 * Los parÃƒÂ¡metros de entrada: blockAddr - direcciÃƒÂ³n del bloque; recvData -
 * leer un bloque de datos MI_OK Valor de retorno: el retorno exitoso MI_OK
 */
 ...
 * blockAddr - direcciÃƒÂ³n del bloque; WriteData - para escribir 16 bytes del
 * bloque de datos Valor de retorno: el retorno exitoso MI_OK
 */
uint8_t RFID::write(uint8_t blockAddr, uint8_t *writeData) {
    ...
    if (status == MI_OK) {
        for (i = 0; i < 16;
             i++)  //?FIFO?16Byte?? Datos a la FIFO 16Byte escribir
        {
            buff[i] = *(writeData + i);
        }
...
inline __attribute__((always_inline)) uint8_t RFID::softSPITranser(
    uint8_t data) {
    uint8_t b = 0;
