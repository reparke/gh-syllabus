...
void setup() {
  ...
  pinMode(PIN_TRIGGER, OUTPUT);  // signal FROM argon to sensor
  pinMode(PIN_ECHO, INPUT);      // signal TO Argon, from sensor
}
...
void loop() {
  ...
  double distanceCM = roundTripTime * 0.03444 / 2;    //round trip

  if (distanceCM == 0 || distanceCM > 78) {
    Serial.println("Out of range");
  }
  else if (distanceCM > 0 && distanceCM < 15) {
    //print warning
  }
  else {
    //print OK and distance
  }
}
