...
int buttonVal;
const int X_MAX = oled.getLCDWidth(); //0-63
const int Y_MAX = oled.getLCDHeight(); //0-47
int xPos = 0;
int yPos = 0;
...
void loop() {
    /* build an etch-a-sketch
        "x" pot and "y" --> control a pixel that is drawn on screen
        button somehow erases the screen

        pot: 0-4095 -> x: 0 - width
        pot: 0-4095 -> y: 0 - height

        read pot values and convert to pixel on the screen

        draw pixel on screen

        add button latch to "clear" the screen

*/
   // map from pot -> x,y
   int xVal = analogRead(PIN_POT0);
   int yVal = analogRead(PIN_POT1);

   //do math
   xPos = map(xVal, 0, 4095, 0, X_MAX);
   yPos = map(yVal, 0, 4095, 0, Y_MAX);

    oled.pixel(xPos, yPos);
    oled.display();

    //we don't necessarily need a latch
