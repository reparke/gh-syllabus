...
void loop() {
    ...
    // int yValue = analogRead(A1);        //0-4095
    // int xMax = oled.getLCDHeight();     //0-47
    // int yMax = oled.getLCDWidth();      //0-95

    // int xPos = map(xValue, 0, 4095, 0, xMax);
    // int yPos = map(yValue, 0, 4095, 0, yMax);

    // oled.clear(PAGE);
    // oled.setCursor(0,0);
    // oled.setFontType(0);
    // oled.print("X: " + String(xPos));
    // oled.setCursor(0, 20);
    // oled.print("Y: " + String(yPos));
    // oled.display();
