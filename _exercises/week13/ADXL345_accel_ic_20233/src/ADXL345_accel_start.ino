...
void changeRgbLed() {
    // adjust the r, g, b leds at the x, y, z accelerations changes
    int r = map(abs(adxl.cx), -0.9, 0.9, 0.0, 255.0);
    int g = map(abs(adxl.cy), -0.9, 0.9, 0.0, 255.0);
    int b = map(abs(adxl.cz), -0.9, 0.9, 0.0, 255.0);
