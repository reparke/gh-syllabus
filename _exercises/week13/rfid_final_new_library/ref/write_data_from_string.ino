 ...
 * Uses MFRC522 - Library to use ARDUINO RFID MODULE KIT 13.56 MHZ WITH TAGS SPI
 * W AND R BY COOQROBOT.
 * -----------------------------------------------------------------------------------------
 ...
 * interface, eg Mifare or NTAG203. The reader can be found on eBay for around 5
 * dollars. Search for "mf-rc522" on ebay.com.
 */
...
void loop() {
    ...
    byte len;
    String word1 = "USC";
    String word2 = "Fight on!";
    for (int i = 0; i < word1.length() && i < 30; i++) {
        buffer[i] = word1.charAt(i);
    }
    ...
    // len = Serial.readBytesUntil('#', (char *)buffer,
    //                             30);  // read family name from serial
    len = word1.length();
    ...
    for (int i = 0; i < word2.length() && i < 30; i++) {
        buffer[i] = word2.charAt(i);
    }

    len = word2.length();

    // len = Serial.readBytesUntil('#', (char *)buffer,
    // 20);  // read first name from serial
