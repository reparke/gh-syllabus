...
void ADXL345::readAccel(int *xyz) { readAccel(xyz, xyz + 1, xyz + 2); }
...
void ADXL345::readFromI2C(byte address, int num, byte _buff[]) {
    ...
    //	Wire.beginTransmission(ADXL345_DEVICE);
    // Wire.reqeustFrom contains the beginTransmission and endTransmission in
    // it.
...
/*************************** SPI BIT STATE **************************/
/*                           ~ GET & SET                            */
bool ADXL345::getSpiBit() { return getRegisterBit(ADXL345_DATA_FORMAT, 6); }
...
/************************* FULL_RES BIT STATE ***********************/
/*                           ~ GET & SET                            */
bool ADXL345::getFullResBit() { return getRegisterBit(ADXL345_DATA_FORMAT, 3); }

// If Set (1) Device is in Full Resolution Mode: Output Resolution Increase with
// G Range
...
/*************************** JUSTIFY BIT STATE **************************/
/*                           ~ GET & SET                            */
bool ADXL345::getJustifyBit() { return getRegisterBit(ADXL345_DATA_FORMAT, 2); }
...
// Enabled. It Accepts a Maximum Value of 255.
...
// Value of 0 May Result in Undesirable Behavior if the Inactivity Interrupt
// Enabled. It Accepts a Maximum Value of 255.
...
// Scale Factor is 62.5mg/LSB.
// Value of 0 May Result in Undesirable Behavior if the Free-Fall interrupt
// Enabled. Accepts a Maximum Value of 255.
...
/************************* SUPPRESS BITS ****************************/
/*                                                                  */
bool ADXL345::getSuppressBit() { return getRegisterBit(ADXL345_TAP_AXES, 3); }
...
/*************************** ASLEEP BIT *****************************/
/*                                                                  */
bool ADXL345::isAsleep() { return getRegisterBit(ADXL345_ACT_TAP_STATUS, 3); }

/************************** LOW POWER BIT ***************************/
/*                                                                  */
bool ADXL345::isLowPower() { return getRegisterBit(ADXL345_BW_RATE, 4); }
...
void ADXL345::setImportantInterruptMapping(int single_tap, int double_tap,
                                           int free_fall, int activity,
                                           int inactivity) {
    if (single_tap == 1) {
...
void ADXL345::setRegisterBit(byte regAdress, int bitPos, bool state) {
    ...
    if (state) {
        _b |=
            (1 << bitPos);  // Forces nth Bit of _b to 1. Other Bits Unchanged.
    } else {
        _b &=
            ~(1 << bitPos);  // Forces nth Bit of _b to 0. Other Bits Unchanged.
    }
...
// Currently supports portrait up, portrait down, landscape left, and landscape right
// Limitation: doesn't currently give correct reading when face up / face down
byte ADXL345::getOrientation() {
    // ref:
    ...
    if (angle >= -2.25 && angle <= -0.75) {
        // landscape left
        return ADXL345_ORIENTATION_LANDSCAPE_L;
    } else if (angle >= -0.75 && angle <= 0.75) {
        // portrai up
        return ADXL345_ORIENTATION_PORTRAIT_U;
    } else if (angle >= 0.75 && angle <= 2.25) {
        // landsape right
        return ADXL345_ORIENTATION_LANDSCAPE_R;
    } else if (angle <= -2.25 || angle >= 2.25) {
        // portrait down
        return ADXL345_ORIENTATION_PORTRAIT_D;
    } else {
        return ADXL345_ORIENTATION_UNKNOWN;
    }
}
