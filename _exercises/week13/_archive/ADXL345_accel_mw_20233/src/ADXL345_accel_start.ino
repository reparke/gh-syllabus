...
void setup() {
    ...
    adxl.setActivityThreshold(150);  // 62.5mg per increment   // Set activity
...
void changeRgbLed() {
    // lets use the acceleration to change the color of the RGB LED

    /*
        ideas:
            - map accel to RGB PWM values
            -
        range of accel: -1 to 1 --> -0.85 to 0.85
        range of RGB: 0 255
            should we use the absolutate of the accel?
    */
    int r = map(adxl.cx, -0.85, 0.85, 0.0, 255.0);
    int g = map(adxl.cy, -0.85, 0.85, 0.0, 255.0);
    int b = map(adxl.cz, -0.85, 0.85, 0.0, 255.0);
...
void loop() {
    ...
    /*
        variables in the adxl
            adxl.x   .y   .z            --> actual acceleration readings
            adxl.cx  .cy    .cz         --> calculated accel values (as a factor
       Gravity)
    */
